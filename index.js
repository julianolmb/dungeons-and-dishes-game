...
const Start = new Room(
  "a quest",
  "you are Jolly Panflip, a young tavern owner and adventurer from the busy town of Royal Port - one of the most important cities in the World of Elandarr. Our story begins when you heard rumours about an old lady, from a small distant town called Nirren, who could make the best pies anyone has ever eaten. You decided to find out everything you could about it, try it yourself, and maybe even bring the recipe to your own tavern. And so you travel to the Town of Nirren, carrying nothing more than a backpack with spare clothes, torches and some food, a real adventure! Type in begin adventure to start your quest."
);
const Nirren = new Room(
  "Town of Nirren",
  "this is a small humble town located miles away from the southern region's capital. Despite it's unremarkable size and wealth, the town has it's charms - lots of colourful buildings, children playing, people laughing and trading. There isn't much to explore, a few houses, a market on the west, a blacksmith on the east and a tavern ahead north."
);
const Tavern = new Room(
  "Tavern",
  "a quite charming and cozy tavern. It seems that this is where all the townsfolk gather around. The inkeeper looks at you, while he polishes a mug."
);
const Market = new Room(
  "Market",
  "a small open street market, despite it's size it has a lot to offer - booths with fresh fruits, vegetables, herbs, eggs, fish, meat, sausages, preserves and honey. Here you can buy amazing ingredients for the pie, if you have the gold."
);
const Blacksmith = new Room(
  "Blacksmith",
  "a simple blacksmith shop, there are weapons and armours all over the place, but nothing fancy. A friendly man looks at you and says: Hello outsider, I'm Maven, and welcome to my shop."
);
const Outskirts = new Room(
  "Outskirts",
  "as you leave the town, you are ready to explore the outskirts ahead of you. Luckily you have a map of the surrounding areas, there seems to be a mountain west from here, and the woods to the east, both not far. In the south there's a road used by the merchants and travelers, it leads to the capital, and to other places."
);
const Mountain = new Room(
  "Mountain",
  "a prominent mountain - with a beautiful landscape with trees and a lake a it's feet. You can further explore the mountain, or go south to visit the lake."
);
const Mines = new Room(
  "Mines",
  "on your explorations your find a mine, you light your torch and go inside. The mines smell terrible, and as you go deeper you find signs of creatures living in there. Goblins! Three of them! You have two choices - to run, or to fight! *future option: fight the goblins*"
);
const Woods = new Room(
  "Woods",
  "the woods are located just a few hours from the town, it's a peaceful and plentyful place. Pretty sure that if you explore it you can find a bunch of wild berries, honey, or even some small animals. *future option: gather ingredients, kill rabit*"
);
const MerchantRoad = new Room(
  "Merchant road",
  "a narrow sand road that leads to the capital, it's mostly used by merchants, but it's far from being busy. You can see a few tracks from caravans, horses and people, most of them seem pretty old. You find a signpost, it points the directions for some locations - east for the Stonebe cave, west for the Pantano swamp and south for the old fort along the road"
);
const Stonebe = new Room(
  "Stonebe cave",
  "this purple stone cave has a very narrow entrance, not befiting for a human to get in. Little can be seen inside, as the light doesn't reach well inside. A putrid smell of something rotten is coming from the inside. It may be difficult to get in, but surely easier than getting out. You can explore the cave if you feel brave enough."
);
const Stonebecave = new Room(
  "Inside the cave",
...
inkeeperBp.addItem(
  new Item(
    "A map of the outskirts.",
    "A simple map describing the outskirts of the town."
  )
);
Inkeeper.backpack = inkeeperBp;
Inkeeper.conversation =
  "hello outsider and welcome to our humble town. So you're looking for this mysterious old lady who makes the best pies in the Elandarr? I might know who you're looking for, but do me a favour first, would you? I know it's a lot to ask, but we never see adventurers here in these parts. One of my shipments was robbed by bandits on the Merchant's Road, it contained a bottle of Jadegreen Whisky, my favourite! Would you bring it back to me? Here's a map to help you on your journeys.";
...
function displayBackpack() {
  ...
  bpItems.forEach(function (item) {
    htmlItems =
      htmlItems + item._name + " (" + item._description + ")" + "<br>";
  });
      ...
      if (directions.includes(command)) {
        ...
        if (currentRoom.name == "Tavern") {
          document.getElementById("game").innerHTML =
            currentRoom.interact("Inkeeper Jones");
        }
      } else if (command == "pickup") {
        if (currentRoom.name == "Tavern") {
          const mapOutskirts = currentRoom.characters[
            "Inkeeper Jones"
          ].backpack.removeItem("A map of the outskirts.");
          Player.backpack.addItem(mapOutskirts);

          document.getElementById("game").innerHTML =
            "Congratulations you just received: " + mapOutskirts.name;
        ...
        document.getElementById("messages").innerHTML =
          "Please write down a valid command.";
      }
    }
  });
};
